...
func PrSh() {
	...
	dsn := "root:123456@tcp(127.0.0.1:3307)/dbName?charset=utf8mb4&parseTime=True&loc=Local"
...
func SshConnect() error {
	...
	sshConfig := &ssh.ClientConfig{
		...
		Auth: []ssh.AuthMethod{
			ssh.Password(config.AppConfig.SSH.Password), // 也可以用 ssh.PublicKeys(...)
		},
		HostKeyCallback: ssh.InsecureIgnoreHostKey(), // 测试用，生产要换安全方式
		Timeout:         time.Second * 5,
	}
	...
	// 建立 SSH 连接
	sshClient, err := ssh.Dial("tcp", fmt.Sprintf("%s:%s", config.AppConfig.SSH.Host, config.AppConfig.SSH.Port), sshConfig)
	if err != nil {
		return err
	}

	// 创建 MySQL 的远程连接转发
	fmt.Println(config.AppConfig.RemoteMySQL)
	mysqlHost := fmt.Sprintf("%s:%s", config.AppConfig.RemoteMySQL.Host, config.AppConfig.RemoteMySQL.Port) // 远程数据库地址
	localPort := "3307"                                                                                     // 本地监听端口
	listener, err := net.Listen("tcp", "127.0.0.1:"+localPort)
	if err != nil {
		return err
	...
	// 使用 GORM 连接本地转发端口（其实访问的是远程数据库）
	for k, v := range config.AppConfig.MySQL {
		// 连接数据库
