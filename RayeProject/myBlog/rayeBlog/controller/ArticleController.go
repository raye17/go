...
func (a ArticleController) Create(c *gin.Context) {
	...
	if err := a.DB.Create(&article).Error; err != nil {
		response.Fail(c, "发布失败", nil)
		return
	}
	response.Success(c, "发布成功", gin.H{
		"id": article.ID,
	})
...
func (a ArticleController) List(c *gin.Context) {
	...
	switch len(args) {
	case 0:
		a.DB.Table("articles").Select("id, category_id, title," +
			" LEFT(content, 80) AS content,head_image,created_at").
			Order((pageNum - 1) * pageSize).Limit(pageSize).Find(&article)
	case 1:
		a.DB.Table("articles").Select("id, category_id, title, "+
			"LEFT(content,80) AS content, head_image, created_at").
			Where(querystr, args[0]).Order("created_at desc").
			Offset((pageNum - 1) * pageSize).Limit(pageSize).Find(&article)
		a.DB.Model(model.Article{}).Where(querystr, args[0]).Count(&count)
	case 2:
		a.DB.Table("articles").Select("id, category_id, title, "+
			"LEFT(content,80) AS content, head_image, created_at").
			Where(querystr, args[0], args[1]).Order("created_at desc").
			Offset((pageNum - 1) * pageSize).Limit(pageSize).Find(&article)
		a.DB.Model(model.Article{}).Where(querystr, args[0], args[1]).Count(&count)
	case 3:
		a.DB.Table("articles").Select("id, category_id, title, LEFT(content,80) AS content,"+
			" head_image, created_at").Where(querystr, args[0], args[1], args[2]).
